...
Actually, there are websites with daily updated information about new malicious URLs, so that one can update the set of denied/allowed connections. However and as the number of websites constantly grows, and some of the updated websites only work by reports made by users, it is impossible for companies to know in advance if an unknown URL is malicious or not. 
% Antonio - The abstract should be quite concise. I would move this paragraph to the introduction (or remove it). 
Thus, in this paper, we propose a system that can initially learn from the already known black/white lists, and then classify a new, unknown, URL connection either as ``should be allowed'' or ``should be denied''. This system is described, as well as its results, and the improvements made by means of an initial data pre-processing 
...
% Antonio - please check the use of the same verbal tense
high accuracies can be obtained with a proper pre-processing of the data, reaching between 96\% and 97\% of correctly classified patterns. 
% Antonio - I would point that the FS improves the system because of the reduction in features, and thus in computational time, while the percentages in accuracy are almost the same (improved sometimes). I mean, the accuracies were very good even without the FS process. ;)
