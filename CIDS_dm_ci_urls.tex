...
Laboratoire de Recherche OpÈrationelle de DÈcision et de ContrÙle de Processus, Institut SupÈrieur de Gestion, Tunisia.\\
S2 Grupo, Spain\\
...
as a step beyond the use of what are known as black/white
...
applying Rough Set Theory for feature selection. We prove that high accuracies can be obtained with
...
\textbf{Keywords.} Computational Intelligence, Rough Sets, Feature Selection, Corporate Security Policies, URL classification, Data Mining.
% Antonio - we should include more security-aimed topics: Internet Access Control, Black/White Lists, and so on
...
In this chapter, we want to improve the accuracies obtained in our previous work \cite{ECTA}, as well as see if the new incorporated method (namely Rough Sets) for feature selection yields to better rules.
...
behaviour after the system decisions \cite{muses_sotics_13}. The results of this work could be applied in this system, by changing the preprocessing step, due to the fact that the database is different. But overall, our conclusions can be scalated to such a system.   
...
% Antonio - Comment the new variable content_type_MCT
...
A reference to each Weka classifier can be found at \cite{Frank2011}. Below are described the top five techniques, obtained from the best results (See Table \ref{tabresults_todos}) of the experiments done in this stage, along with more specific bibliography. Na√Øve Bayes method \cite{Bayesian_Classifier_97} has been included as a baseline, normally used in text categorization problems. According to the results, the five selected classifiers are much better than this method.
...
\hline
NaÔve Bayes & 92.30 & 90.77 $\pm$ 0.06 & 91.77 \\
\cline{1-1}
...
\multicolumn{1}{l|}{} & Random (mean) & Sequential & Random (mean) & Sequential \\ 
\hline
NaÔve Bayes & 93.01 $\pm$ 0.32 & 82.61 & 93.09 $\pm$ 0.91 & 83.04 \\ 
\cline{1-1}
...
\hline
NaÔve Bayes & 92.76 $\pm$ 1.34 & 77.74 \\ 
\cline{1-1}
...
Focusing on the showed rules, it can be noticed that almost all also depend on very specific features/values, such as \texttt{server\_or\_cache\_address}, or even on the \texttt{client\_address} (not showed here).

These features create several non-useful rules, mainly in the case of the client IP address. Because we can not settle that an specific IP could not access to some URLs.

Thus, we have conducted a pair of additional experiments in this line, just removing these three features (\texttt{url}, \texttt{server\_or\_cache\_address}, \texttt{client\_address}) from the dataset and training again the classifiers.
These have been performed over the unbalanced data, considering a 10-fold cross validation process.
...
% ***
% RESUMEN en un p√°rrafo de lo que se ha obtenido:
% - muy buenos porcentajes (en torno al 95-97\%) en general incluso en datos no balanceados
% - el m√©todo pierde efectividad si se omiten algunos patrones que pueden no haber aparecido antes (datos secuenciales o undersampling) => hay que estudiarlo mejor en siguientes trabajos
% 
% Esto ten√≠ais que haberlo puesto antes que nada. - JJ
% Antonio - yo creo que est√° bien as√≠. Las conclusiones suelen ser autocontenidas y yo las estructuro como un resumen del trabajo. ;)
% So we can conclude that...
% *** la propuesta ha sido exitosa y podr√≠a ser, potencialmente, una buena herramienta de decisi√≥n en sistemas de seguridad ***
