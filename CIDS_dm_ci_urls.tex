...
patterns. Furthermore, we also prove that including the use of Computational Intelligence techniques for pre-processing the data enhances the system performance, in terms of speed, while the accuracies remain close to 97\%.
...
insecure situations.  % Please introduce subsections in an
                      % argument. First, we will explain this
                      % because... then, we will explain that in
                      % subsection because - JJ
...
techniques depending on the data. %Avoid repetitions of "Data at hand"
                                %- JJ  
...
file. We realised % remarquer in French is "realize" in English - Jj
% And I've changed it to "realise", because British 
...
data. %something on what you are going to use and why - JJ 
...
%Outline
%\subesction{Builiding the dataset}
%\subsubsection{Security rules parsing}  
%\subsubsection{URL log data parsing}   
%\subsection{Balancing the dataset}  
%\subsection{Pre-prepocessing the dataset}  
%\subsubsection{Elimination of the redan….}
...
As it can be seen from Table \ref{tabresults_nobalan}, all five classifiers achieved a high performance classifying in the right way the test dataset. Also, having low values of standard deviation means that the obtained accuracies are stable; and this can be seen from the obtained results as well. 
...
\hline
Na\"ive Bayes & 92.76 $\pm$ 1.34 & 77.74 \\
\cline{1-1}
...
\caption[Correctly classified patterns for unbalanced data after the removal of entries that could lead to misclassification.]{Percentage of correctly classified patterns for unbalanced data, after the removal of entries that could lead to misclassification.. (a) 80\% for training, \%20 for testing, and 90\% for training, 10\% for testing. (b) 60\% for training, 40\% for testing. Best results are marked in boldface. \label{tab:repurl_unb_traintest}}
\end{table}
