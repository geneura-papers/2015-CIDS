...
The classifier that lightly stands out over the others is \textit{Random Forest}, being the best in almost every case for randomly made divisions, and it also has good results for sequentially made divisions. However, if we focus on the standard deviation, \textit{REP Tree} is the chosen one, as its results present robustness (low standard deviation).
