...
% En un universo lejano vivía nuestro rasputín.
% 
% 
% pantalones
% camiseta
...
Next step in the `database maintenance' is that having the reduced dataset (see previous Subsection) with 12 features, our aim now is to see if we can reduce execution time while preserving good accuracies. Thus, after applying a feature selection method, based on Rough Sets (see Section \ref{sec:featureselection}), we obtained a list of 9 features. Figure \ref{fig:data_9features} shows the final list of selected features.
